...
const SOCIAL_ICONS: Record<string, (p: { className?: string }) => React.JSX.Element> = {
  facebook: ({ className }) => (
...
export function Footer(props: FooterProps) {
  ...
  return (
    <footer className="w-full border-t border-divider bg-surface-raised">
        ...
        <div className="site-container py-12 md:py-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-16">
            ...
            <div className="space-y-4">
              {logoUrl ? (
                <img src={logoUrl} alt={companyName ?? ''} className="h-8 w-auto" loading="lazy" />
              ) : companyName ? (
                <p className="font-display font-bold text-lg text-ink">{companyName}</p>
              ) : null}

              <div className="space-y-2 text-sm text-ink-muted font-body">
                {address && (
                ...
                {phone && (
                  <a href={`tel:${phone}`} className="flex items-center gap-2 hover:text-ink transition-colors">
                    <Phone className="w-4 h-4 flex-shrink-0" />
                    {phone}
                  </a>
                )}
                {email && (
                  <a href={`mailto:${email}`} className="flex items-center gap-2 hover:text-ink transition-colors">
                  ...
                  {socialLinks.map((link) => {
                    ...
                    return (
                      <a
                        ...
                        className="w-9 h-9 flex items-center justify-center rounded-site bg-surface-elevated text-ink-muted hover:text-ink hover:bg-surface-base transition-colors"
                        aria-label={link.platform}
                      >
                        {Icon ? <Icon className="w-4 h-4" /> : <span className="text-xs">{link.platform.charAt(0)}</span>}
                      </a>
                    );
                  })}
                </div>
            ...
            {links.length > 0 && (
              <div>
                <p className="site-label mb-4">Enlaces</p>
                <ul className="space-y-2">
                  {links.map((link) => (
                    <li key={link.href}>
                      <a
                        href={link.href}
                        className="text-sm text-ink-muted font-body hover:text-ink transition-colors"
                      >
                        {link.label}
                      </a>
                    </li>
          ...
          <div className="site-section">
            <div className="site-container py-5 flex items-center justify-between">
              <p className="text-xs text-ink-faint font-body">
                &copy; {new Date().getFullYear()} {companyName ?? 'Corredor'}. Todos los derechos reservados.
              </p>
              <p className="text-xs text-ink-faint font-body">
                Hecho con Corredor
              </p>
